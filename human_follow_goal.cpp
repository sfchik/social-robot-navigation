...
void HumanFollowGoal::callbackPeoplePose(const pedsim_msgs::TrackedPersons::ConstPtr& msg)
{
	...
	if(tracked_ == false) {
		for(char i = 0; i < msg->tracks.size(); i++) {
			...
			diff_x = msg->tracks[5].pose.pose.position.x - pos_robot_x;
			diff_y = msg->tracks[5].pose.pose.position.y - pos_robot_y;
...
//			ROS_INFO("x=%.2f y=%.2f len=%.2f deg=%.2f", diff_x, diff_y, len, deg);
		...
		diff_x = msg->tracks[tracked_id].pose.pose.position.x - pos_robot_x;
		if(diff_x > 0.0) {
			quat = tf::createQuaternionMsgFromRollPitchYaw(0.0, 0.0, 3.142);

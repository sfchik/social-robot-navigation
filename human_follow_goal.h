...
class HumanFollowGoal
{
	public:
		...
		move_base_msgs::MoveBaseGoal goalFakeCreation(move_base_msgs::MoveBaseGoal goal_fake); 
	...
	private:
		...
		enum {explore_ = 0, ped_follow_, ped_fake_follow_, reached_} state;

		ros::NodeHandle nh_;
		ros::Subscriber sub_people_pos_;
		ros::Subscriber sub_robot_pos_;

  	move_base_msgs::MoveBaseGoal goal, goal_prev, goal_temporal, goal_store_ori, goal_store_fake;
		float leader_angle_generic;  //a generic variable for leader_angle_explore_max & leader_angle_shift_max
		...
		float leader_angle_explore_max, leader_angle_shift_max, leader_distance_max, leader_speed_max, leader_speed_min;
		int leader_id;
};
